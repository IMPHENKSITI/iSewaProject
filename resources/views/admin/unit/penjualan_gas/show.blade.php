@extends('admin.layouts.admin')

@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <!-- Breadcrumb -->
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">Unit Layanan / Penjualan Gas /</span> Detail Produk Gas
        </h4>

        <!-- Detail Card -->
        <div class="row">
            <div class="col-xl-15">
                <div class="card mb-4 shadow-sm border-0">
                    <div class="card-body">
                        <div class="d-flex align-items-start">

                            <!-- Gambar Carousel -->
                            <div class="me-4 flex-shrink-0" style="width: 380px;">
                                <div class="position-relative">
                                    <div id="carouselExample{{ $gas->id }}" class="carousel slide" data-bs-ride="carousel">
                                        <div class="carousel-inner">
                                            <div class="carousel-item active">
                                                <img src="{{ asset('storage/' . $gas->foto) }}" class="card-img-top"
                                                    alt="{{ $gas->jenis_gas }}"
                                                    style="height: 300px; object-fit: cover; object-position: center; border-radius: 8px;">
                                            </div>
                                            @if ($gas->foto_2)
                                                <div class="carousel-item">
                                                    <img src="{{ asset('storage/' . $gas->foto_2) }}" class="card-img-top"
                                                        alt="{{ $gas->jenis_gas }}"
                                                        style="height: 300px; object-fit: cover; object-position: center; border-radius: 8px;">
                                                </div>
                                            @endif
                                            @if ($gas->foto_3)
                                                <div class="carousel-item">
                                                    <img src="{{ asset('storage/' . $gas->foto_3) }}" class="card-img-top"
                                                        alt="{{ $gas->jenis_gas }}"
                                                        style="height: 300px; object-fit: cover; object-position: center; border-radius: 8px;">
                                                </div>
                                            @endif
                                        </div>

                                        <!-- Tombol Prev/Next -->
                                        <button class="carousel-control-prev" type="button"
                                            data-bs-target="#carouselExample{{ $gas->id }}" data-bs-slide="prev">
                                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                            <span class="visually-hidden">Previous</span>
                                        </button>
                                        <button class="carousel-control-next" type="button"
                                            data-bs-target="#carouselExample{{ $gas->id }}" data-bs-slide="next">
                                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                            <span class="visually-hidden">Next</span>
                                        </button>
                                    </div>
                                </div>
                            </div>

                            <!-- Info Detail & Tombol Aksi -->
                            <div class="flex-grow-1" style="min-width: 0;"> <!-- Menambahkan min-width untuk mencegah kolaps -->
                                <h5 class="card-title fw-semibold">{{ $gas->jenis_gas }}</h5>
                                <p class="text-muted mb-4">{{ $gas->deskripsi }}</p>

                                <div class="mb-3">
                                    <strong>Harga Satuan:</strong> <span class="text-danger fs-5">Rp. {{ number_format($gas->harga_satuan, 0, ',', '.') }}</span>
                                </div>

                                <div class="row g-3 mb-4">
                                    <div class="col-md-6">
                                        <strong>Stok Tersedia:</strong> {{ $gas->stok }} {{ $gas->satuan }}
                                    </div>
                                    <div class="col-md-6">
                                        <strong>Status:</strong> {{ ucfirst($gas->status) }}
                                    </div>
                                    <div class="col-md-6">
                                        <strong>Kategori:</strong> {{ $gas->kategori }}
                                    </div>
                                    <div class="col-md-6">
                                        <strong>Lokasi:</strong> {{ $gas->lokasi }}
                                    </div>
                                </div>

                                <!-- Tombol Aksi -->
                                <div class="d-flex gap-2 align-items-end">
                                    <a href="{{ route('admin.unit.penjualan_gas.edit', $gas->id) }}"
                                        class="btn btn-warning btn-sm">Ubah</a>
                                    <a href="{{ route('admin.unit.penjualan_gas.index') }}"
                                        class="btn btn-secondary btn-sm">Kembali</a>
                                    <form action="{{ route('admin.unit.penjualan_gas.destroy', $gas->id) }}" method="POST"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus gas ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@push('styles')
<style>
    
    .card {
        transition: all 0.2s ease;
    }
    .card:hover {
        transform: translateY(-3px);
        box-shadow: 0 8px 20px rgba(0,0,0,0.08);
    }
    .carousel-control-prev-icon,
    .carousel-control-next-icon {
        width: 30px;
        height: 30px;
        opacity: 0.8;
    }
    .carousel-control-prev-icon:hover,
    .carousel-control-next-icon:hover {
        opacity: 1;
    }
    .btn-sm {
        padding: 0.375rem 0.75rem;
        font-size: 0.875rem;
    }
</style>
@endpush